...
const { data: analyticsData, status } = await useFetch<any>('/api/admin/ga/overview')

const stats = computed(() => {
  if (!analyticsData.value) return []
  const totals = analyticsData.value.totals
  return [
    { label: 'Active Users', value: totals[0]?.value || '0', icon: 'i-lucide-users', color: 'text-blue-500' },
    { label: 'Sessions', value: totals[1]?.value || '0', icon: 'i-lucide-zap', color: 'text-amber-500' },
    { label: 'Page Views', value: totals[2]?.value || '0', icon: 'i-lucide-file-text', color: 'text-emerald-500' },
    {
      label: 'Bounce Rate',
      value: ((totals[3]?.value || 0) * 100).toFixed(1) + '%',
      icon: 'i-lucide-percent',
      color: 'text-rose-500',
    },
    {
      label: 'Avg Session',
      value: Math.round(totals[4]?.value || 0) + 's',
      icon: 'i-lucide-clock',
...
const maxBarValue = computed(() => {
  ...
  return Math.max(...rows.map((r: any) => Number(r.metricValues?.[0]?.value) || 1), 1)
})
</script>
  ...
  <div class="max-w-6xl mx-auto py-8 px-4 flex flex-col gap-8">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-dimmed">Visitor behavior for austin-texas.net · Last 30 days</p>
      </div>
    </div>
    ...
    <div v-else-if="!analyticsData" class="h-96 flex flex-col items-center justify-center text-dimmed">
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <UCard v-for="stat in stats" :key="stat.label">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-dimmed font-medium uppercase tracking-wider">{{ stat.label }}</span>
            <UIcon :name="stat.icon" class="size-4" :class="stat.color" />
          </div>
          <span class="text-2xl font-bold tabular-nums">{{ Number(stat.value).toLocaleString ? Number(stat.value).toLocaleString() : stat.value }}</span>
        </UCard>
      </div>
      ...
      <UCard>
        ...
        <div v-if="analyticsData.rows?.length" class="h-48 flex items-end gap-[2px] px-2">
          <div
            ...
            :style="{
              height: `${Math.max((Number(row.metricValues?.[0]?.value) / maxBarValue) * 100, row.metricValues?.[0]?.value > 0 ? 2 : 0)}%`,
            }"
          >
            <div
