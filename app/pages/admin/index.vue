...
const tools = [
  { label: 'Search Radar', to: '/admin/radar', icon: 'i-lucide-radar', color: 'text-amber-500', desc: 'Keyword discovery & content gaps' },
  { label: 'Maps Tester', to: '/admin/apple-maps', icon: 'i-lucide-map-pin', color: 'text-emerald-500', desc: 'Apple Maps API testing' },
  { label: 'Grid Crawler', to: '/admin/grid-crawler', icon: 'i-lucide-grid-3x3', color: 'text-violet-500', desc: 'Neighborhood grid scanning' },
]

const analytics = [
  { label: 'Search Console', to: '/admin/gsc', icon: 'i-lucide-search', color: 'text-blue-500', desc: 'Organic search performance' },
  { label: 'Google Analytics', to: '/admin/analytics', icon: 'i-lucide-bar-chart-3', color: 'text-orange-500', desc: 'Visitor behavior & traffic' },
  { label: 'PostHog', to: '/admin/posthog', icon: 'i-lucide-zap', color: 'text-rose-500', desc: 'Product analytics & recordings' },
]
</script>
  ...
  <div class="max-w-6xl mx-auto py-8 px-4 flex flex-col gap-8">
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
      ...
      <div class="flex items-center gap-3">
        <div v-if="user" class="flex items-center gap-3 px-4 py-2 rounded-xl bg-elevated border border-default">
        ...
        <UButton color="error" variant="soft" icon="i-lucide-log-out" size="sm" @click="handleLogout">
          Sign Out
        </UButton>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div class="size-10 rounded-xl bg-default flex items-center justify-center group-hover:scale-110 transition-transform">
              <UIcon :name="item.icon" class="size-5" :class="item.color" />
            </div>
            <UIcon name="i-lucide-arrow-up-right" class="size-4 text-dimmed opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
