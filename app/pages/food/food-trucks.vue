...
const config: MapPageConfig = {
  ...
  description:
    'Find the best food trucks in Austin, TX. Interactive map of the top 10 trucks — tacos, Thai, kolaches, and everything in between.',
  introText:
    'Austin\'s food truck scene is legendary — trailers that spawn empires and flavors you can\'t find in any restaurant. <strong class="text-default">Tap any pin</strong> to find your next favorite truck.',
  parentCategory: 'food',
  parentLabel: '← Food',
  parentPath: '/food/',
...
const spots = computed<MapSpot[]>(() => {
  ...
  if (apiSpots && apiSpots.length > 0) {
    return apiSpots.map((s, i) => ({
      ...
      category: s.category || 'Food Truck',
