...
const config: MapPageConfig = {
  ...
  description: 'Find the best restaurants in Austin, TX. Interactive map of the top 10 — from James Beard winners to neighborhood gems.',
  introText: 'Austin\'s dining scene punches far above its weight. From James Beard-winning kitchens to hidden neighborhood gems — <strong class="text-default">tap any pin</strong> to explore the city\'s best tables.',
  parentCategory: 'food',
  parentLabel: '← Food',
  parentPath: '/food/',
...
const spots = computed<MapSpot[]>(() => {
  ...
  if (apiSpots && apiSpots.length > 0) {
    return (apiSpots as any[]).map((s, i) => ({
      ...
      category: s.category || 'Restaurant', description: s.description || '',
