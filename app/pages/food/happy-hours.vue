...
const config: MapPageConfig = {
  ...
  description: 'Find the best happy hours in Austin, TX. Interactive map of the top 10 spots — craft cocktails, $4 pints, and half-off small plates.',
  introText: 'Austin\'s happy hour scene goes way beyond cheap wells. From craft cocktail bars to brewery patios — <strong class="text-default">tap any pin</strong> to find the best deals in town.',
  parentCategory: 'food',
  parentLabel: '← Food',
  parentPath: '/food/',
...
const spots = computed<MapSpot[]>(() => {
  ...
  if (apiSpots && apiSpots.length > 0) {
    return (apiSpots as any[]).map((s, i) => ({
      ...
      category: s.category || 'Happy Hour', description: s.description || '',
