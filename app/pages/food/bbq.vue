...
const config: MapPageConfig = {
  ...
  description: 'Find the best BBQ in Austin, TX. Interactive map of the top 10 spots — from Franklin\'s legendary brisket to Valentina\'s Tex-Mex BBQ fusion.',
  introText: 'Austin is the barbecue capital of the world. From East Side legends to suburban smokehouses — <strong class="text-default">tap any pin</strong> to explore the best brisket, ribs, and sausage in town.',
  parentCategory: 'food',
  parentLabel: '← Food',
  parentPath: '/food/',
...
const spots = computed<MapSpot[]>(() => {
  ...
  if (apiSpots && apiSpots.length > 0) {
    return (apiSpots as any[]).map((s, i) => ({
      ...
      category: s.category || 'BBQ', description: s.description || '',
