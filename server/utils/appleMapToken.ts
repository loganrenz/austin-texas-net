 ...
 */
export async function getAppleMapsAccessToken(event?: any): Promise<string> {
  ...
  const preSignedJwt = (config as any).mapkitServerApiKey as string
  ...
  const privateKeyPem = (config as any).appleSecretKey as string
  const teamId = (config as any).appleTeamId as string
  const keyId = (config as any).appleKeyId as string
