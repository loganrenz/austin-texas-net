/* eslint-disable @typescript-eslint/no-explicit-any -- Drizzle rowsAffected + OpenAI response parsing */
/**
...
export default defineEventHandler(async (event) => {
  ...
  for (const n of neighborhoods) {
    ...
    updatedCount += (res1 as any).rowsAffected || 0
    ...
    updatedCount += (res2 as any).rowsAffected || 0
  }
...
Only return data for the IDs provided. Match against the canonical lists if possible.`
      ...
      if (res.ok) {
        ...
        for (const r of results) {
          ...
          updatedCount += (updateRes as any).rowsAffected || 0
        }
      }
    }
  }
